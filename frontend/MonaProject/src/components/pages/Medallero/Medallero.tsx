...
import { getMedalTable } from '../../../services/medallero';
import type { MedalRow } from '../../../services/medallero';
...
const columns: TableColumn<MedalRow>[] = [
  ...
  { name: 'Oro', selector: (row: MedalRow) => row.gold, center: true, sortable: true, width: '80px' },
  { name: 'Plata', selector: (row: MedalRow) => row.silver, center: true, sortable: true, width: '80px' },
  { name: 'Bronce', selector: (row: MedalRow) => row.bronze, center: true, sortable: true, width: '110px' },
  { name: 'Puntos', selector: (row: MedalRow) => row.points, center: true, sortable: true, width: '100px',
    cell: (row: MedalRow) => <span className={styles.pointsCell}>{row.points}</span> },
];
...
const Medallero: React.FC = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const sortedData = await getMedalTable();
